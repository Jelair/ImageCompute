...
- (void)drawLinearGradient:(CGContextRef)context{
    ...
    /*绘制线性渐变
     ...
     startPoint:起始位置
     endPoint:终止位置
     options:绘制方式，kCGGradientDrawsBeforeStartLocation开始位置之前就进行绘制，到结束位置之后不再绘制，kCGGradientDrawsAfterEndLocation开始位置之前不进行绘制，到结束点之后继续填充
     */
...
- (void)drawRectWithLinearGradientFill:(CGContextRef)context{
    ...
    //裁切出一块矩形用于显示，注意必须先裁切再调用渐变
    //CGContextClipToRect(context, CGRectMake(20,50,200,300));
    //裁切还可以使用UIKit中对应的方法
