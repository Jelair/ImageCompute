...
//  BasicImage
//
//  Created by NowOrNever on 14/10/2017.
...
@interface MeiTuViewController ()<UINavigationControllerDelegate,UIImagePickerControllerDelegate>{
    UIImagePickerController *_imagePickerController;//照片选择控制器
...
- (void)initLayout{
    ...
    //饱和度（默认为1，大于饱和度增加小于1降低）
    UILabel *lbSaturation = [[UILabel alloc] initWithFrame:CGRectMake(10, 10, 60, 25)];
    lbSaturation.text = @"Sauration";
    lbSaturation.font = [UIFont systemFontOfSize:CONSTROLPANEL_FONTSIZE];
    [controlView addSubview:lbSaturation];
    UISlider *sldStaturation = [[UISlider alloc] initWithFrame:CGRectMake(80, 10, 230, 30)];//UISlider高度虽然无法调整，很多朋友会说高度设置为0即可，实际上在iOS7中设置为0后是无法拖动的
...
//    _context = [CIContext contextWithOptions:option];
    ...
    /*使用GPU渲染，推荐，但注意GPU的CIContext无法跨应用访问，例如直接在UIImagePickerController的完成方法中调用上下文处理就会自动降级为CPU渲染，所以推荐现在完成方法中保存图像，然后在主线程序中调用
     */
...
- (void)setImage{
    ...
    _imageView.image = [UIImage imageWithCGImage:temp];//转为CGImage显示在界面中
